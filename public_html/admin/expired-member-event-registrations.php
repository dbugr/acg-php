...
if ($cust_id == NULL) {
  ...
  $query = "
          select members.m_club,members.cust_id, members.m_firstname, 
          members.m_lastname, reserve.r_attending, 
          reserve.reserve_id,
          events.event_id,events.e_name, events.e_begindate 
  ...
  while ($m_row = mysqli_fetch_array($result)) {
    ...
    if (
      expired_membership($MemberExpirationDates[$cust_id])
      and
      ($today <=  $m_row['e_begindate'])
    ) {
      $str = "<tr>"
        ...
        . 'method="post" '
        . 'action="'
        . '/admin/deletereserveYN.php'
        . '"> '
        . '<input type="hidden" name="reserve_id" value="'
        . $m_row['reserve_id']
        . '">'
        . '<input type="submit" value="Delete Reservation ID: '
        . $m_row['reserve_id']
        . '">'
        . "</form>"
        . "</td>"
...
$FileName = __FILE__;
$WebPageTitle = 'Admin Delete Expired Member Reservations '; // . $ClubCompanyName;
$admin = true;
require('top.php');
...
<div id="centercontent">
  ...
  <p>List of former (expired) members who still have reservations for events</p>
  <table border=1>
