...
if ($cust_id == NULL) {
  ...
  $query =
    "select members.cust_id, members.m_firstname, 
          ...
          members.m_club, members.m_email,
          count(*) as reserve_count  
          from members inner join reserve 
          on members.cust_id = reserve.cust_id 
          where reserve.r_attending = 'Yes' 
          group by members.cust_id 
          order by reserve_count asc";
...
$WebPageTitle = 'Admin Event Reservations Report ' . $ClubCompanyName;
$admin = true;
require('top.php');
